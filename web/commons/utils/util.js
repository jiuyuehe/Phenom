 ...
 * });
    ...
    util.getBrowserAndVersions = function () {
        var Sys = {};
        var ua = navigator.userAgent.toLowerCase();
    ...
    util.userNumConvert=function(direction,num){
        if (!num || num.length <= 0 || num === '--')
            return "-";
        return _.numberFormat(num, 0) + "人";
    }
    ...
    util.getItemPreviewUrl = function (model) {
        if (model.isFolder()) {
            return '#' + model.get("diskType") + "/open/" + model.id;
        }
